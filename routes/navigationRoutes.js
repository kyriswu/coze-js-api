...
router.get('/docs/volcengine/seedance-2-0', (req, res) => {
    ...
    const doc = {
        ...
        description: '创建和轮询接口均不支持免费试用：必须提供创建该任务时使用的有效本项目 API Key。轮询请优先通过 X-API-Key 请求头传递，避免将密钥放入 URL。创建任务前，所有图片、视频和音频参考链接都会验证为可公开访问的 HTTP(S) 媒体；不可访问、非公开网络地址、重定向异常或类型不匹配会被拒绝并返回素材位置和原因。接口会移除本地 api_key 后透传其余业务字段到火山方舟；content 与 tools 也支持以 JSON 字符串提交。任务完成时会在查询响应中返回最终扣除积分或欠费状态。任务对象由上游直接透传，因此以实际 data 内容为准。',
        requestParams: [
            ...
            { name: 'content[].<media>.url', type: 'string', required: '媒体素材时必填', desc: '公开可访问的 HTTP(S) 素材链接；创建前会检测可访问性和媒体类型，不通过时不会调用上游', example: 'https://example.com/first-frame.png' },
  ...
  -H "X-API-Key: uk_live_xxx"`,
        ...
        responseFields: [
            ...
            { path: 'settlement.status', type: 'string', desc: '仅成功且已返回用量的任务出现：charged 表示已结算，outstanding 表示视频已返回但仍有欠费，pending 表示正在结算' },
            { path: 'settlement.credits', type: 'number', desc: '该成功任务的最终扣除积分' },
        ],
        responseSample: JSON.stringify({
            code: 200,
            msg: '查询视频生成任务成功',
