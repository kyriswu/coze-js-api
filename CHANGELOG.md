...
- GPT-Image-2 的两个生图入口现于调用上游前原子扣除 3 积分；并发请求无法再同时通过余额预检后超额调用上游。进入生图流程后若上游失败，积分按既定规则不退款。
- 应用 Redis 已迁至本机 `lite-chat-redis` 的隔离 DB 1；Compose 不再定义或依赖项目专属 Redis 服务。公开 Redis 读写、枚举和删除接口已下线。
- 新增 Seedance 2.0 视频生成公开文档页，覆盖创建任务、参考媒体、任务轮询和 cURL 示例。
- Seedance 2.0 成功任务现根据上游返回的完成 token 结算最终积分；轮询必须使用创建任务的同一 API Key。余额不足时仍返回已生成视频，并标记为欠费。
- Seedance 2.0 创建任务现会在上游调用前校验参考图片、视频和音频链接：仅允许可公开访问的 HTTP(S) 媒体，并拒绝内网地址、异常重定向、超时和类型不匹配的素材。
- Seedance 2.0 的任务创建和任务查询上游超时统一延长至 10 分钟，且不自动重试创建请求。
